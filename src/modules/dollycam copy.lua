...
function m.reconnectPoints()
    local pathdir = workspace:FindFirstChild(m.pathfoldername)
    for _, i in pairs(pathdir:GetDescendants()) do
        if i:IsA("BasePart") and not i.Locked then
            i.Changed:Connect(m.RenderPath)
        end
        if i:IsA("Folder") and i.Name ~= m.renderfoldername and i.Name ~= m.pointfoldername then
